...
C-----------------------------------------------------------------------
      ...
      INTEGER N, NA
      REAL(RP) AR(N,NA),AI(N,NA),BR(N,NA),BI(N,NA)
 
C-----------------------------------------------------------------------
C DECALRATIONS FOR OUTPUT VARIABLES
C-----------------------------------------------------------------------
 
      REAL(RP) ALFR(N),ALFI(N),BETA(N)
      REAL(RP) ZVR(N,NA), ZVI(N,NA)
      INTEGER IFAIL
...
C
      ...
      REAL(RP) AR(NM,N),AI(NM,N),BR(NM,N),BI(NM,N),
     &         ZR(NM,N),ZI(NM,N)
      REAL(RP) R,S,T,TI,U1,U2,XI,XR,YI,YR,RHO,U1I
      LOGICAL  MATZ
      REAL(RP) ZERO,ZONE
C
...
C
     ...
     X       BETA(N),ZR(NM,N),ZI(NM,N)
      REAL(RP) R,S,A1,A2,EP,SH,U1,U2,XI,XR,YI,YR,
...
C
   ...
   30 CONTINUE
C
      IF (ANORM .EQ. ZERO) ANORM = 1.0_RP
      IF (BNORM .EQ. ZERO) BNORM = 1.0_RP
      EP = EPS1
...
C     ********** DETERMINE SHIFT **********
  ...
  124 B3344 = B33 * B44 - B33I * B44I
     ...
     X     - AR(NA,NA) * BI(NA,EN) - AI(NA,NA) * BR(NA,EN)
      ...
      YR = (A33 - SH) / 2.0_RP
      YI = (A33I - SHI) / 2.0_RP
      Z3 = sqrt(DCMPLX(YR**2-YI**2+XR,2.0_RP*YR*YI+XI))
      U1 = DBLE(Z3)
      U1I = DIMAG(Z3)
...
C
     ...
     X       ALFR(N),ALFI(N),
     X       BETA(N),ZR(NM,N),ZI(NM,N)
      REAL(RP) R,T,RI,TI,XI,ALMI,ALMR,BETM,EPSB
      COMPLEX(RP) Z3
      REAL(RP) ZERO
C
C
