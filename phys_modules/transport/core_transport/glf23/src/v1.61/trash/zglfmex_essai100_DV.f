...
c 
c Compile this mexfile on Deneb using : /usr/local/matlab5/bin/mex -f mexopts_glf.sh zglfmex_essai100_DV.f blas_zgeev.o callglf3d.o  glf2d.o r8tomsqz.o zgeev.o xtverb.o
...
c pointeurs et tailles      
      ...
      CRONOSINT zpte_m_pr, zpti_m_pr, zpne_m_pr, zpni_m_pr
...
C log Te gradient profile 
      zpte_m_pr = mxGetPr(prhs(29))
      call mxCopyPtrToReal8(zpte_m_pr,zpte_m,jpd+1)
C log Ti gradient profile 
      zpti_m_pr = mxGetPr(prhs(30))
      call mxCopyPtrToReal8(zpti_m_pr,zpti_m,jpd+1)
C log ne gradient profile 
      zpne_m_pr = mxGetPr(prhs(31))
      call mxCopyPtrToReal8(zpne_m_pr,zpne_m,jpd+1)
C log ni gradient profile 
      zpni_m_pr = mxGetPr(prhs(32))
      call mxCopyPtrToReal8(zpni_m_pr,zpni_m,jpd+1)
...
c
      ...
      if (impur.eq.0.D0) then
          ...
          idengrad = 2  ! one main hydrogenic ion + one impurity;PB idengrad = 3 GIVES PROBLEMS TO THE GLF23 SOLVER ???
      endif
...
c      bt_exp       = 1.0
      ...
      bteff_exp    = 1.0  ! effective B-field (used when bt_flag > 0), dummy ?
c      rmajor_exp   = 1.0
c      amassgas_exp = 1.0
c      zimp_exp     = 6.0
c      amassimp_exp = 12.0
...
c      x_alpha      = 1.   ! alpha stabilization
      zpte_in      = 0.     ! dummies (gradient profile cannot be a scalar !!!, F.I.)
      zpti_in      = 0.
      zpne_in      = 0.
      zpni_in      = 0.
c
...
c gradients prescrits par Cronos (igrad = 1) 
c        zpte_m(j) = 0.0
c        zpti_m(j) = 0.0
c        zpne_m(j) = 0.0
c        zpni_m(j) = 0.0
...
c------------ MAIN CALL ----------------------------------------------
c
        call callglf3d( leigen, nroot, iglf
     & , jshoot, jmm, jmaxm, itport_pt
     & , irotstab, te_m, ti_m, ne_m, ni_m, ns_m
     & , igrad, idengrad, zpte_m, zpti_m, zpne_m, zpni_m
     & , angrotp_exp, egamma_exp, gamma_p_exp, vphi_m, vpar_m, vper_m
