...
************************************************************************
     ...
     > zpte_m,        ! externally provided log gradient te w.r.t rho (i_grad=1)
...
c
      do j=0,jmaxm
        if(i_grad.eq.0) then       ! initialize only if gradients are computed
           zpte_m(j) = 0.D0
           zpti_m(j) = 0.D0
           zpne_m(j) = 0.D0
           zpni_m(j) = 0.D0
...
c 
      if(i_grad.eq.0) then
        ...
        enddo
      endif   ! if i_grad = 1, reads directly the gradients in zpt_m
...
c   Original version, not valid if there is a profile of the gradients
c        if(i_grad.eq.1) then
c         zpmte=zpte_in
c         zpmti=zpti_in
c         zpmne=zpne_in
c         zpmni=zpni_in
