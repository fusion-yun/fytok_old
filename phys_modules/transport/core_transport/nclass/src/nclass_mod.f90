...
!Get lu decomposition of response matrix
        iflag=0
!        write(*,*) 'lancement de nclass_decomp i=',i,'msnc= ',ms_nc
!        write(*,*) 'akk=',akk(1,1),' calmi=',calmi_nc(1,1,im),im
        CALL NCLASS_DECOMP(akk,k_order_nc,indxk,d,iflag)
...
!Source and response from poloidal source (p' and T') terms 
        ...
        DO k=1,k_order_nc !Over velocity moments
          ...
          if (k .eq. 3 .and. i .eq. 5) then
!            write(*,*) 'ymus_nc(k,1,i)=',ymus_nc(k,1,i)
!            write(*,*) 'srcth(1,i)=',srcth(1,i)
!            write(*,*) 'grppiz_nc(im,iza)=',grppiz_nc(im,iza)
!            write(*,*) 'den_iz(im,iza)=',den_iz(im,iza),im,iza
!            write(*,*) 'cc=',cc,iz
           endif
        END DO !Over velocity moments
         if (im .eq. 1) then
!           write(*,*) 'xis_nc(i)=',xis_nc(i),i
!           write(*,*) 'rk6s(:,4,i)=',rk6s(:,4,i)
!           write(*,*) 'crk6i(1,4,im)=',crk6i(1,4,im)
...
!           write(*,*) 'crk6i(3,4,6)=',crk6i(3,4,im),xis_nc(i),i
        endif
...
!        write(*,*) 'k_order_nc=',k_order_nc,im,iza,i
!        write(*,*) 'rk6s=',rk6s(1:k_order_nc,6,i)
!      CALL mexErrMsgTxt('stop')
...
!External force
          ...
          if (m1 .eq. 1) then
!            write(*,*) 'crk6i(m,4,im)=',crk6i(m,4,im)
!            write(*,*) 'im=',im,' xki3(1,1)=',xki3(1,1)
...
!-------------------------------------------------------------------------------
      iflag=0
!      write(*,*) ''
      CALL NCLASS_DECOMP(akiki,k_order_nc*mi_nc,indxki,d,iflag)
...
!Evaluate species flows 
      DO k=1,3 !Over forces
        if (k .eq. 1) then
!          write(*,*) ' avant backsub, xki3(1,1)=',xki3(1,1)
         endif
...
!               write(*,*) 'calnii_nc(1,l,im,jm)=', calnii_nc(1,l,im,jm)
!               write(*,*) 'im=',im,' jm=',jm,' l1=',l1,' m=',m
!                 write(*,*) 'xk(l)= ',xk(l),jm,l1
!                 write(*,*) 'xki3(l1,m)= ',xki3(l1,m)
!                 write(*,*) 'calnii_nc(1,l,im,jm)=',calnii_nc(1,l,im,jm)                 
              endif
            ENDDO !Over velocity moments
...
!                  write(*,*) 'upars_nc(1,m,i)=',upars_nc(1,m,i),m,i
!                  write(*,*) 'xk(l)=',xk(l),'l= ',l
!                  write(*,*) 'rk6s(1,l,i)=',rk6s(1,l,i)
                endif
            ENDDO !Over velocity moments
...
!Currents
        ...
        if (i .eq. 1 .and. (ipr .eq. 6 .or. ipr .eq. 4)) then
!          write(*,*) 'ipr= ',ipr
!          write(*,*) 'denzc= ',denzc,z_coulomb
!          write(*,*) 'pjbbs_nc= ',pjbbs_nc/z_coulomb/1000
!          write(*,*) 'upars_nc(1,1,i)= ',upars_nc(1,1,i)
        endif
        !<J_ex.B>
...
!      write(6,*) 'flow, qfls_nc(1,i)=',qfls_nc(1,i),i
!      write(6,*) 'flow, upars_nc(1,1,i)=',upars_nc(1,1,i)
        !<E.B> 
...
!      write(*,*) 'petap_nc =',petap_nc,1.0_rspec,pjboh_nc
...
!-------------------------------------------------------------------------------
      ...
      SUBROUTINE NCLASS_DECOMP(a,n,indx,d,iflag)
!-------------------------------------------------------------------------------
