...
def transport_glf23(equilibrium: Equilibrium, core_profiles: CoreProfiles, core_transport: CoreTransport):
        ...
        z_beta = 2*z_mu0/bt_exp**2*(ne_m*(te_m +  (zeff_exp - 1.0)/zimp_exp/(zimp_exp - 1.0)*  Ti_m) + ni_m*ti_m)*1.60219e3
        ...
        egamma_d
        ) =   glf2d(GLF23.leigen, GLF23.nroot, GLF23.iglf 
                           , GLF23.jshoot, GLF23.jmm, GLF23.jmaxm, GLF23.itport_pt 
                           , GLF23.irotstab, te_m, ti_m, ne_m, ni_m, ns_m 
                           , GLF23.igrad, GLF23.idengrad, zpte_in, zpti_in, zpne_in, zpni_in 
                           , angrotp_exp, egamma_exp, gamma_p_exp, vphi_m, vpar_m, vper_m 
                           , zeff_exp, bt_exp, GLF23.bt_flag, rho 
                           ...
                           , GLF23.x_alpha_exb, GLF23.x_alpha, GLF23.i_delay 
        ...
        if True:
            ...
            core_transport.heat_fluxes[i,k] = -profiles_rm.ni[i,k]*core_transport.chieff[i,k]* zpti_in*z_kev2Joule*geometry_rm.grho2[i]*geometry_rm.vprime[i]

            # Effective main ion particle fluxes
            core_transport.deff[i,k] = diff_m[i]
            core_transport.particle_fluxes[i,k] = -core_transport.deff[i,k]*zpni_in*geometry_rm.grho2[i]* geometry_rm.vprime[i]*1.0e19
            ...
            if (GLF23.l_vd_shift) : # THEN
                ...
                if (F_BDS/G_BDS <= 0.2) : # THEN
                    ...
                    core_transport.vp[i,k] = ((F_BDS + core_transport.d[i,k]*geometry_rm.grho2[i]*zpni_in)/core_profiles.ni[i,k])/geometry_rm.grho1[i]
            ...
            if (core_profiles.nions == 2) : # THEN
                ...
                zpnq_in = (nq_m[i] - nq_m(i - 1) + sqrt(epsilon(1.0)))/drho # Added for additional impurity information PIS 2007-11-20
                ...
                core_transport.particle_fluxes[i,k] = -core_transport.deff[i,k]*zpnq_in*geometry_rm.grho2[i]*geometry_rm.vprime[i]*1.0e19
                ...
                if (GLF23.l_vd_shift) : # THEN
                    ...
                    if (F_BDS/G_BDS <= 0.2) : # THEN
                        core_transport.d[i,k] = 0.2*core_transport.chi[i,k]
                        core_transport.vp[i,k] = ((F_BDS + core_transport.d[i,k]*geometry_rm.grho2[i]*zpnq_in)/core_profiles.ni[i,k])/geometry_rm.grho1[i]
            ...
            core_transport.heat_fluxes[i,k] = -profiles_rm.ne[i]*core_transport.chieff[i,k]*   zpte_in*z_kev2Joule*geometry_rm.grho2[i]*geometry_rm.vprime[i]
            ...
            if (core_profiles.nions == 2) : # THEN
                core_transport.deff[i,k] = (diff_m[i]*zpni_in + zimp_exp[i]*diffz_m[i]*zpnq_in)/zpne_in # By ambipolarity
            else:
                core_transport.deff[i,k] = diff_m[i]
            #END if
            ...
            core_transport.particle_fluxes[i,k] = -core_transport.deff[i,k]*zpne_in*geometry_rm.grho2[i]*geometry_rm.vprime[i]*1.0e19
            ...
            if (GLF23.l_vd_shift) : # THEN
                ...
                if (F_BDS/G_BDS <= 0.2) : # THEN
                    ...
                    core_transport.d[i,k] = 0.2*core_transport.chi[i,k]
                    core_transport.vp[i,k] = ((F_BDS + core_transport.d[i,k]*geometry_rm.grho2[i]*zpne_in)/core_profiles.ne[i])   /geometry_rm.grho1[i]
