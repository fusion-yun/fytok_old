...
c external arrays
     ...
     >  rmajor_exp, ! FK -  made radially dependent -- zimp_exp, amassimp_exp, 
     ...
     >  zimp_exp(0:jmaxm), amassimp_exp(0:jmaxm), !FK made radially dependent
...
c some internals
     ...
     >       xnimp_jp1, xnimp, zimp_jp1, vnewk3x
...
c  and alpha(jm) depends on zpxx_m(jm+1)
 200  format(i2,2x,0p1f5.2,0p5f10.5)
c
      do j=jm-jptf,jm+jpt
c
...
c      amassimp_gf=12.D0
       zimp_gf=zimp_exp(jm)
       zimp_jp1=zimp_exp(jm+1)
       amassimp_gf=amassimp_exp(jm)
c       zimp_gf=zimp_exp         ! made radially depende FK
c       amassimp_gf=amassimp_exp ! FL made radially dependent
       rlnimp_gf=1.D0
       zpmnimp=1.D0
       if (idengrad.eq.2) dil_gf=1.D0-nim/nem
       if (idengrad.eq.3) then
         apwt_gf=nim/nem
         aiwt_jp1=(zeff_exp(jm+1)*ne_m(jm+1)-ni_m(jm+1)
     >            -ns_m(jm+1))/(zimp_jp1**2*ne_m(jm+1))
         xnimp_jp1=aiwt_jp1*ne_m(jm+1)
         aiwt_gf=(zeff_exp(jm)*ne_m(jm)-ni_m(jm)
     >           -ns_m(jm))/(zimp_gf**2*ne_m(jm))
...
c diffznem added by F. Imbeaux 08/04/2004, following Kinsey's instructions
      ...
      diffznem=cmodel*gfac*diff_im_gf*cgyrobohm_m(jm)
...
c
      if ( itport_pt(1) .eq. 0 ) then
         diffnem = 0.D0
         diffznem = 0.D0 ! added by F. Imbeaux 08/04/2004
      endif
...
c
      diff_m(jm)=diffnem
      diffz_m(jm)=diffznem  ! added by F. Imbeaux 08/04/2004
      chie_m(jm)=chietem
      chii_m(jm)=chiitim
