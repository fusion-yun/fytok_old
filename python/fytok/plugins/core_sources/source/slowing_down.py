...
class SlowingDown(CoreSources.Source):
    ...
    def update(self, *args,   equilibrium: Equilibrium.TimeSlice,  core_profiles_1d: CoreProfiles.Profiles1d,     **kwargs) -> float:

        ionHe = core_profiles_1d.ion["He"]

        electrons = core_profiles_1d.electrons

        nAlpha = ionHe.density_fast

        src_slowing_down = nAlpha/tau_sd

        self.profiles_1d.current.ion.insert(
            {"label": "alpha", "particle": -src_slowing_down})
        self.profiles_1d.current.ion.insert(
            {"label": "He", "particles": src_slowing_down})

        self.profiles_1d.current.electrons["temperature"] = P_alpha
