...
class RadialGrid:
    def __init__(self, axis: np.ndarray = None, label: str = "rho_tor_norm", equilibrium=None) -> None:
        ...
        if label == "psi_norm":
            self._psi_norm = axis if axis is None else self._parent.profiles_1d.psi_norm
        else:
            if axis == None:
                axis = np.linspace(0, 1.0, len(self._parent.profiles_1d.psi_norm))
            attr = getattr(self._parent.profiles_1d, label, None)
            if attr is None:
                raise AttributeError(f"Can not find attribute {label}!")
            self._psi_norm = Function(attr, self._parent.profiles_1d.psi_norm)(axis)
    ...
    def rho_tor_norm(self):
        return self._parent.profiles_1d.rho_tor_norm(self._psi_norm)

    @cached_property
    def rho_tor(self):
        return self._parent.profiles_1d.rho_tor(self._psi_norm)

    @cached_property
    def rho_pol_norm(self):
        return self._parent.profiles_1d.rho_pol_norm(self._psi_norm)

    @cached_property
    def psi(self):
        return self._parent.profiles_1d.psi(self._psi_norm)

    @cached_property
    def volume(self):
        return self._parent.profiles_1d.volume(self._psi_norm)

    @cached_property
    def area(self):
        return self._parent.profiles_1d.area(self._psi_norm)

    @cached_property
    def surface(self):
        return self._parent.profiles_1d.surface(self._psi_norm)
