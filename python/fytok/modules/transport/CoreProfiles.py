...
class CoreProfiles(Profiles):
    ...
    class Profiles1D(Profiles):
        ...
        def electrons(self):
            ...
            return CoreProfiles.Profiles1D.Electrons(self["electrons"], axis=self.grid.rho_tor_norm, parent=self)

        @cached_property
        def ion(self):
            """Quantities related to the different ion species"""
            return List(self["ion"], default_factory=lambda d, axis=self.grid.rho_tor_norm, parent=self, **kwargs: CoreProfiles.Profiles1D.Ion(d, axis=axis, parent=parent, **kwargs),  parent=self)

        @cached_property
        def neutral(self):
            """Quantities related to the different neutral species"""
            return List(self["neutral"],
                        default_factory=lambda d, axis=self.grid.rho_tor_norm, parent=self, **kwargs: CoreProfiles.Profiles1D.Neutral(d, axis=axis, parent=parent, **kwargs), parent=self)
        ...
        def e_field(self):
            ...
            return CoreProfiles.Profiles1D.EField(self["e_field"], axis=self.grid.rho_tor_norm, parent=self)
    ...
    def global_quantities(self):
        return CoreProfiles.GlobalQuantities(self["global_quantities"], axis=self.time, parent=self)
