...
class FluxSurface:
    ...
    def surface_mesh(self):
        ...
        for i, r in enumerate(self.coordinate_system.grid.dim1):
            if abs(r) < self.tolerance:
                R[i, :] = R0
                Z[i, :] = Z0
                continue
            psival = r*(self.psi_boundary - self.psi_axis)+self.psi_axis
            for j, t in enumerate(self.coordinate_system.grid.dim2):
                theta = t+theta0
                r0 = R0
                z0 = Z0
                r1 = R0 + Rm * sin(theta)
