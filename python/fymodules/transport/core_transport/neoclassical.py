...
class NeoClassical(CoreTransport.Model):
    ...
    def refresh(self, *args,
        ...
        nZ2_imp = np.sum([(imp.z_ion_1d(rho_tor_norm)**2)*imp.density(rho_tor_norm)
                          for imp in core_profiles_1d.ion if imp.is_impurity])
        ...
        c1 = np.array((4.0+2.6*ft_e)/(1.0+1.02*np.sqrt(nu_star_e)+1.07*nu_star_e)/(1.0 + 1.07 * epsilon32*nu_star_e))
        c3 = np.array((7.0+6.5*ft_e)/(1.0+0.57*np.sqrt(nu_star_e)+0.61*nu_star_e) /
                      (1.0 + 0.61 * epsilon32*nu_star_e) - c1*5/2)
        ...
        for ion in core_profiles_1d.ion:
            ...
            if not ion.is_impurity:
                ...
                chi_i = (0.66*(1.0+1.54*alpha)+(1.88*np.sqrt(epsilon)-1.54*epsilon)
                         * (1.0+3.75*epsilon))/(1.0+1.03*np.sqrt(mu_star_i)+0.31*mu_star_i)
            ...
            c2 = c1*Ti/Te
            c4 = ((-1.17/(1.0+0.46*ft_i) + 0.35*np.sqrt(nu_star_i)) / (1 + 0.7*np.sqrt(nu_star_i)) + 2.1*e3n2) \
                / (1 - e3n2) / (1 + e3n2)*c2
