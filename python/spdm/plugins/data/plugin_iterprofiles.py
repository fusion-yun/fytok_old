...
def load_core_source(profiles, grid, R0: float, B0: float = None):
    ...
    Q_e = (profiles["Poh"].values
           ...
           )*1e6/scipy.constants.electron_volt
...
def read_iter_profiles(path):
    ...
    Q_DT = (profiles_1D["Peic"].values
            + profiles_1D["Pdti"].values
            + profiles_1D["Pibm"].values
            )*1e6/scipy.constants.electron_volt

    Q_He = (- profiles_1D["Pdti"].values
            - profiles_1D["Pdte"].values
            )*1e6/scipy.constants.electron_volt
