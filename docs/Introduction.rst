...
|Tokamak| 是一个由多个复杂子系统耦合而成的大科学系统装置。每个子系统中都蕴含了具有不同时空尺度、不同物理机制的复杂过程。
这些物理过程各自成体系，又相互紧密联系。很难用单一数值模型描述整个复杂体系的多时空尺度过程。所以，需要分别对各个子系统上
的不同物理过程进行建模，形成针对单独子系统的数值模型；并使子系统间的相互作用以数据的方式在模型中传递；最终，将子系统数值
模型整合在一起形成全装置的“模拟器”，一个“*数字*”的 |Tokamak| 孪生。
...
Background 背景
=================
|Tokamak| 放电建模的首要问题是对等离子体芯部输运问题的求解。针对不同子系统和物理过程的模拟，通过输运模型耦合在一起
形成整个装置的数值模型。早期的输运模拟可以追溯到上世纪六十年代，B.B.Kadomtsev和 O.P.Pogutse首次系统讨论这一问题 :cite:`leontovich_turbulence_1970`。
当时 Yu. N. Dnestrovski 和  D. P. Kostomarov 开发出了最早的输运程序  :cite:`dnestrovskii_abstracts_1969,dnestrovskii_numerical_2011`。
...
+---------------------------------------------------------------+----------------------------------------------+------+-----------------------+----------------------+-----------------------------------------+
| IMAS  :cite:`imbeaux_design_2015`                             |  ITER                                        | 2011 |         /             |          /           | Kepler,Python,Fortran,C...              |           
+---------------------------------------------------------------+----------------------------------------------+------+-----------------------+----------------------+-----------------------------------------+
...
“ |Tokamak| 模拟器”，例如 Europen Transport Simulator (ETS)。近年来，“集成建模”程序的用途更加多样化，工具也越发丰富，
以工具集的形式呈现，如Integrated Modelling & Analysis Suite(IMAS) 等。

集成建模平台的涉及到 |Tokamak| 物理和装置的各个方面，应用范围广泛。其开发离不开高层的项目支持和协调。目前，国家级别集成
建模合作项目计划主要有三个，
...
方便的交换或传递大量的机构化数据变量。 IDS也可以作为装置数据库的入口，将原本散落在各处的数据汇集到单一的访问路径下。
IMAS标准化的数据模型设计过程，特别强调数据结构应与装置无关，以保持其通用性，ITER只是数据模型的一个实例。

现有的程序可以通过增加与IMAS数据模型的接口，或者数据转换适配器（adaptor）实现与IMAS的集成和数据交换。并能通过IMAS，以访问
统一的接口访问多个装置的实验数据库。新开发的程序也应围绕着IMAS数据模型构建数据交互界面。
